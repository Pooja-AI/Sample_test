...
const {
  ...
  const CorrosionQuest = ({ onClose }) => {
    const questions = ["Can you explain Functional failures along with an example.",
   "Can you tell me something about failure modes?",
   "What is the procedure to access a failure effect?",
    "list down the steps to modify a failure",
    "give an overview on RCM and FMEA workflow",
    "what are the system functions?",
    "what are the strategies to create analysis from template",
     "List down the ways in which functional failure could happen."
     ]
  ...
  const handleUserMessage = async (message) => {
    try {
      ...
      const response = await axios.post('http://52.157.248.201:5000/chatBot', {
        message,
  ...
  const handleProcessButtonClick = async () => {
    try {
      ...
      const response = await axios.post('http://52.157.248.201:5000/uploadFiles', formData, {
        headers: {
    ...
    <div className="main-sop">
     <div className='top-apm' style ={{backgroundColor:'rgba(32, 35, 66, 1)'}}>
      ...
      <div className="container-sop">
        <div className="left-section-sop">
          
          <div className="chatbot-container-sop">
              <div className='Assistant-sop'>
              <div className='assis-sop'>Asset Documentation</div>
          ...
          <div className='right-section-sop'>
          {infoModal && (
            <PopupModal
